...
document.addEventListener("DOMContentLoaded", () => {
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            ...
            // Estrae il nome del gruppo dalla parte prima della @ e lo converte in maiuscolo
            const gruppoUtente = user.email.split('@')[0].toUpperCase();
            
            // Imposta il gruppo e inizializza l'app
            impostaGruppo(user.email, gruppoUtente);
            loadingScreen.classList.add('d-none');
            mainContent.classList.remove('d-none');
            inizializzaApp(gruppoUtente);
        } else {
...
// Funzione per impostare le informazioni del gruppo nell'interfaccia
function impostaGruppo(email, gruppo) {
    document.getElementById('emailUtente').textContent = email;
    document.getElementById('gruppoAssegnato').textContent = gruppo;
    document.getElementById('nomeGruppo').textContent = gruppo;
    document.getElementById('gruppo').value = gruppo;
}

// Funzione per inizializzare l'applicazione
function inizializzaApp(gruppoUtente) {
    // Popola Anno
    ...
    get(dbRef)
        .then((snapshot) => {
            if (snapshot.exists()) {
                ...
                // Reset solo della tabella ZADANKAI, mantieni i dati PRATICANTI
